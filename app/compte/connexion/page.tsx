...
import { faUser, faLock, faSpinner, faCheckCircle, faExclamationCircle, faShoppingBag } from '@/src/lib/icons';
...
export default function CompteConnexionPage() {
  ...
  const handleSignup = async (e: FormEvent) => {
    ...
    if (password.length < 8) {
      setIsError(true); setMsg('Le mot de passe doit faire au moins 8 caractères.'); return;
    }
    setBusy(true);
    ...
    if (error) {
      ...
      setIsError(false);
      setMsg('Compte créé ! Vérifiez votre boîte mail pour confirmer votre adresse.');
                ...
                <form onSubmit={tab === 'login' ? handleLogin : handleSignup} className="flex flex-col gap-4">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder={tab === 'login' ? 'Mot de passe' : 'Mot de passe (min. 8 caractères)'}
                      required
                      className="w-full rounded-xl py-3 pl-10 pr-4 text-sm outline-none transition-colors"
                      style={{ border: '1.5px solid rgba(200,134,10,0.25)', background: '#FFFAF8' }}
                      onFocus={(e) => { e.currentTarget.style.borderColor = '#C8860A'; e.currentTarget.style.boxShadow = '0 0 0 3px rgba(200,134,10,0.1)'; }}
