          ...
          <td style="padding:28px 32px 0;">
            <div style="background:#C0392B22;border-left:4px solid #C0392B;padding:12px 16px;border-radius:0 8px 8px 0;">
              <p style="margin:0;font-size:16px;font-weight:bold;color:#1C0A0A;">
                ❌ Annulation commande #${shortId}
              </p>
              <p style="margin:4px 0 0;font-size:13px;color:#555;">Le client a annulé sa commande.</p>
            </div>
          </td>
        </tr>
        <tr>
        ...
        <tr>
          <td style="padding:0 32px 8px;">
            <div style="background:#fff3cd;border-left:4px solid #ffc107;padding:14px 16px;border-radius:0 8px 8px 0;">
              <p style="margin:0 0 4px;font-size:13px;font-weight:bold;color:#856404;">Motif d'annulation</p>
  ...
  try {
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to:      [adminEmail],
        subject: `❌ Annulation #${order.id.slice(0, 8).toUpperCase()} — ${order.customer_name}`,
