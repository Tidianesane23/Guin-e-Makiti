      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;max-width:560px;width:100%;">
        <tr>
          <td style="background:#1C0A0A;padding:24px 32px;">
            <p style="margin:0;font-size:20px;font-weight:bold;color:#fff;">
              ...
              <span style="float:right;font-size:13px;color:rgba(255,255,255,0.5);font-weight:normal;margin-top:4px;">Litige livraison</span>
            </p>
          </td>
        </tr>
        <tr>
          <td style="padding:28px 32px 0;">
            <div style="background:#C0392B22;border-left:4px solid #C0392B;padding:12px 16px;border-radius:0 8px 8px 0;">
              <p style="margin:0;font-size:16px;font-weight:bold;color:#1C0A0A;">
                ⚠️ Litige — commande #${shortId} marquée livrée
              </p>
              <p style="margin:4px 0 0;font-size:13px;color:#555;">Le client affirme ne pas avoir reçu sa commande.</p>
            </div>
          </td>
        </tr>
        <tr>
        ...
        <tr>
          <td style="padding:0 32px 8px;">
            <div style="background:#fff3cd;border-left:4px solid #ffc107;padding:14px 16px;border-radius:0 8px 8px 0;">
              <p style="margin:0 0 4px;font-size:13px;font-weight:bold;color:#856404;">Message du client</p>
...
export async function POST(req: NextRequest) {
  try {
    const { order, reason } = await req.json();
    ...
    if (!res.ok) {
      ...
      console.error('[notify-dispute] Resend error:', err);
      return NextResponse.json({ error: err }, { status: 500 });
    }

    return NextResponse.json({ sent: true });
  } catch (err) {
    console.error('[notify-dispute] Exception:', err);
